...
import com.example.r2d.adapter.TemplateAdapter
...
class AlertDialogHelper {
    ...
    companion object{
        ...
        @JvmStatic
        fun dialogShowTemplateList(context: Context,title:String?, list:ArrayList<TemplateData>, CB : (TemplateData) -> Unit) {
            ...
            // set list
            recyclerViewList?.layoutManager = GridLayoutManager(context, 1)
            var templateAdapter = TemplateAdapter(context, list) {
                CB(it)
                dialog.dismiss()
            }

            recyclerViewList?.adapter = templateAdapter
